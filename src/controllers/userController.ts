...
async function createUser(request:Request, response:Response)
{
    try
    {
        ...
        //Compatibilizando os dados vindos da requisição http, que por
        //padrao vem todos como string
        let data = request.body;
        data.premium = (data.premium == true)
...
async function setPremium(request:Request, response:Response)
{
    const {id} = request.params;
    try
    {
        ...
        if(user)
        {
            ...
            response.status(201).json(userPremium);
        }
        else
            response.status(404).json({'error': 'Usuario não cadastrado'})
    }
    catch(error)
    {
        response.status(500).json({error:error})
